    ...
    constructor(bankName, abbrevation) {
      ...
      static createBank(bankName){
        try {
            ...
            let abbrevation=bankName.split(' ').map(x=>x.charAt(0)).join('')
      ...
      #updateBankNameAndAbbrevation(value){
        if (typeof value!='string'){
            throw new Error ('Invalid bank parameter ')
        }
        this.bankName=value
        this.abbrevation=value.split(' ').map(x=>x.charAt(0)).join('')
      }
     static updateBank(bankId,parameter,value){
            if(typeof parameter!='string'){
                throw new Error ('Invalid parameter')
            }
            switch (parameter) {
                case 'bankName':this.#updateBankNameAndAbbrevation(value)
                    break;
        ...
        removeBank(indexOfBank){
          return Bank.allBanks.splice(indexOfBank,1)
        }
      }   
     module.exports=Bank
